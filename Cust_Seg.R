...
monetary
###################
grid.arrange(recency, frequency, monetary)
...
italianart<-ggplot(book_data,aes(ItalArt))+
  geom_histogram(binwidth=1)+
  theme_bw()+
  facet_wrap(~Florence)+  
  labs(title="Histogram of Art of Florence Sold by the number of Genaeral Art Books sold",x="Frequency",subtitle="0=Florence Not Purchased and 1=Florence Purchased" )
italianart
######################
##################
...
###########################
cluster<-ggplot(book_data_clusters, aes(x=Florence))+
  geom_bar()+
  labs(title="Counts of Purchase of Art History of Florence By CLuster",subtitle="1=Cluster 1 and 2=Cluster 2")+
  theme_bw()+
  facet_wrap(~cluster)
cluster
#####################
gender_1<-ggplot(book_data_clusters, aes(x=cluster))+
  geom_bar()+
  labs(title="Counts of Cluster By Gender",subtitle="0=Male and 1=Female")+
  theme_bw()+
  facet_wrap(~Gender)
gender_1
grid.arrange(cluster, gender_1)
#####################Box plots

boxplot(book_data$M ~ k$cluster,
        xlab='Cluster', ylab='Dollars',
        main='Monetary Value by Cluster')

boxplot(book_data$R ~ k$cluster,
        xlab='Cluster', ylab='Days',
...
boxplot(book_data$FirstPurch ~ k$cluster,
        xlab='Cluster', ylab='Days Passed',
        main='Firstpurch by Cluster')

boxplot(book_data$ChildBks ~ k$cluster,
        xlab='Cluster', ylab='Number Purchased',
...
boxplot(book_data$ItalCook ~ k$cluster,
        xlab='Cluster', ylab='Number Purchases',
